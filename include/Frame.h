...
{
    cv::Point2f  p, q;
    std::vector<float> lineEq2d;
    std::vector<std::array<float, 128>> desc;
} FrameLine;
...
public:
    ...
    Frame(const cv::Mat &imGray, Ort::Session& session, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
...
public:
    ...
    void input_from_model(cv::Mat input_mat, vector<vector<vector<float>>>& junc, vector<vector<vector<float>>>& heat, vector<vector<vector<float>>>& desc, Ort::Session& session);
};
