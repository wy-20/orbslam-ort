...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, Ort::Session& session, const double &timestamp, string filename)
{
    mImGray = im;
    ...
    if (mSensor == System::MONOCULAR)
    {
        ...
        // TODO input onnx model
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray, session,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        else
            mCurrentFrame = Frame(mImGray, session,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
    }
