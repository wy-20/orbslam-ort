...
#include "line/line_extractor.h"
#include "line/line_match.h"
#include <thread>
...
Frame::Frame(const cv::Mat &imGray, Ort::Session& session, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     ...
     mpCamera2(nullptr)
{
    ...
    mTimeORB_Ext = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(time_EndExtORB - time_StartExtORB).count();
#endif
//    // TODO add line etract

    vector<vector<vector<float>>>_junctions(65, vector<vector<float>>(47, vector<float>(155)));
    vector<vector<vector<float>>>_heatmaps(2, vector<vector<float>>(376, vector<float>(1240)));
    vector<vector<vector<float>>>_descriptions(128, vector<vector<float>>(94, vector<float>(310)));

    input_from_model(imGray, _junctions, _heatmaps, _descriptions, session);
    cout << " out ok";
    ...
    auto line_seg = lext.line_detector(_junctions, _heatmaps);

    // 采样点
    vector<vector<pair<float, float>>> line_points(line_seg.size(), vector<pair<float, float>>(num_samples, {0, 0}));
    // 采样点标记
    vector<vector<bool>> valid_points(line_seg.size(), vector<bool>(num_samples, false));
    // 得到线描述子
    auto line_des = lext.line_descriptor(line_seg, _descriptions, line_points, valid_points);
    int n = line_seg.size();
    mLine.resize(n);
    for(int i = 0; i < n; i++)
    {
        int x1 = line_seg[i].first.first, y1 = line_seg[i].first.second;
        int x2 = line_seg[i].second.first, y2 = line_seg[i].second.second;
        cv::Point2f p = cv::Point2f(x1, y1);
        cv::Point2f q = cv::Point2f(x2, y2);
        vector<float> lineEq2d(3);
...
//        std::array<float, 128> desc = line_des[i];
        vector<std::array<float, 128>> desc = line_des[i];
        FrameLine tmp;
        tmp.p = p;
        tmp.q = q;
        tmp.lineEq2d = lineEq2d;
        tmp.desc = desc;
        mLine[i] = tmp;
    }
...
// TODO onnx input
void Frame::input_from_model(cv::Mat input_mat, vector<vector<vector<float>>>& junc, vector<vector<vector<float>>>& heat, vector<vector<vector<float>>>& desc, Ort::Session& session)
{
